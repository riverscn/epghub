...
class Program:
    ...
    """
    def __init__(self, title: str, start_time: datetime, end_time: datetime, channel_id: str, desc: str = '', episode: str = '', sub_title: str = '') -> None:
        self.title = title
    ...
    def __eq__(self, other) -> bool:
        if isinstance(other, Program):
            return self.start_time == other.start_time and self.end_time == other.end_time and self.title == other.title
        return False
    ...
    def __str__(self) -> str:
        return f'Program({self.channel}, {self.title}, {self.start_time}, {self.end_time})'
...
class Channel:
    ...
    """
    def __init__(self, id: str, metadata: dict = {}, update_callable: Callable[[Any, date], bool] | None = None) -> None:
        self.__id = id
        self.metadata = metadata
        self.metadata.update({'last_update': datetime(
    ...
    def now_playing(self, now: datetime = datetime.now()) -> Program | None:
        ...
        for program in self.programs:
            if program.start_time.astimezone() <= now.astimezone() <= program.end_time.astimezone():
                return program
        return None
    ...
    def flush(self, date) -> None:
        ...
        """
        self.programs = [program for program in self.programs if program.start_time.date() != date]
        return None
